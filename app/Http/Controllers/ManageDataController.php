        ...
        try {
            ...
            if ($id == '0') {
                ...
                $query->where("flag", 7)
                    ->whereNULL("call_back")
                    ->whereNULL("followup")
                    ->where("status", "Ready for Renewal");
            } else {
                $query->whereRaw('0');
            }
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!empty($request->alternate_no)) {
                ClientData::where('parent_id', $request->parent_id)
                    ->update([
            ...
            if ($request->calling_status == 'Not Connected') {
                ...
                foreach ($data as $row) {
                    ...
                    ClientData::where('parent_id', $row['parent_id'])
                        ->update([
                            'call_back' => NULL,
                            'followup' => NULL,
                ...
                $connectStatusfollow = ['Customer Disconnected the call', 'Due Later'];
                $connectStatusrenewal = ['Follow Up - Quotation Sent', 'Follow Up - Ready for Payment', 'Follow Up - Ready for Renewal', 'Ready for Renewal - Dealer'];
                ...
                if ($request->connect_status == 'Call Back') {
                    ...
                    return back()->with('success', 'Details Updated Successfully');
                } else if (in_array($request->connect_status, $connectStatusrenewal)) {
                    $data = ClientData::where('parent_id', $request->parent_id)->get()->toArray();
                    foreach ($data as $row) {
                        AgentInput::Insert(
                            [
    ...
    public function call(Request $request)
    {
        try {
            ...
            if (!is_null($pri_no)) {
                ...
                if (!is_null($respon)) {
                    $data = ClientData::where('parent_id', $parent_id)->get()->toArray();
                    foreach ($data as $row) {
                        ClickToCall::insert([
                            "data_id" => $row['id'],
                            "agent_id" => $email,
